...
import { AppEntitiesServiceProxy, AppEntityAttachmentDto, AppEntityDto, AppEntityExtraDataDto, GetAllEntityObjectTypeOutput, GetSycAttachmentCategoryForViewDto, SycAttachmentCategoryDto, SycEntityObjectTypesServiceProxy } from '@shared/service-proxies/service-proxies';
...
export class CreateOrEditAppEntityDynamicModalComponent extends AppComponentBase implements AfterViewInit {
    ...
    show(entityObjectType: { name: string, code: string },appEntity?:AppEntityDto,): void {
        ...
        if (this?.appEntity?.id) {
            this.editMode = true
            this._appEntitiesServiceProxy.getAppEntityForEdit(this.appEntity.id)
                .subscribe((res) => {
    ...
    save(form: NgForm) {
        ...
        this.saving = true;
        this._appEntitiesServiceProxy.saveEntity(this.appEntity)
    ...
    getExtrAttributes() {
        this._sycEntityObjectTypesServiceProxy.getAllWithExtraAttributesByCode(this.entityObjectType.code)
        .subscribe(async (result) => {
            if (result.length > 0) {
                ...
                if (this.selectedItemTypeData?.extraAttributes?.extraAttributes) {
    ...
    async setAdditionalAndRecommendedExtraAttributes() {
        const extraAttributres = this.selectedItemTypeData.extraAttributes.extraAttributes
        this.extraAttributes = this._extraAttributeDataService.getFilteredAttributesByUsage(extraAttributres,undefined,false)
        const isLookupExtraAttributes : FilteredExtraAttribute[] = this.extraAttributes.filter(extraAttr => extraAttr.isLookup)
        const isLookupExtraAttributesCodes : string[] = isLookupExtraAttributes.map(extraAttr=>extraAttr.code)
        const responses = await this._extraAttributeDataService.getExtraAttributesLookupDataAsync(isLookupExtraAttributesCodes)
        if(responses) {
    ...
    setSelectedAppEntityExtraDataOnEditMode() {
        ...
        this.appEntity.entityExtraData.forEach((ItemExtraData) => {
            ...
            let value = isLookup ? ItemExtraData.attributeValueId : ItemExtraData.attributeValue
            if (!selectedExtraDataAsObject[key]) selectedExtraDataAsObject[key] = []
            isLookup ? selectedExtraDataAsObject[key].push(value) : selectedExtraDataAsObject[key] = value
        })

        this.extraAttributes.map((elem) => {
            let _selectedValue = selectedExtraDataAsObject[elem.attributeId]
            if (_selectedValue !== undefined) {
                if(elem.isLookup && !elem.acceptMultipleValues) elem.selectedValues = _selectedValue[0]
    ...
    askForAnotherAdd(event) {
     ...
     // extra attribute
     multiValuesExtraAttributeOnChange($event :{itemValue:number,value:number[],originalEvent:MouseEvent},extraAttrDefinition:FilteredExtraAttribute<number[]>){
        ...
        if( isJustAdded ) {
            ...
            this.appEntity.entityExtraData = this.appEntity.entityExtraData.filter(elem=>elem.attributeValueId != selectedAttrValue)
        }
    }

    singleValueExtraAttributeOnChange($event :{value:number,originalEvent:MouseEvent},extraAttrDefinition:FilteredExtraAttribute<number>){

        let selectedAttrValue = $event.value

        if(!this.appEntity.entityExtraData)  this.appEntity.entityExtraData = []

        const currentExtraAttrIndex:number = this.appEntity.entityExtraData.findIndex(elem=>elem.attributeId == extraAttrDefinition.attributeId)
        ...
        if(currentExtraAttrIndex > -1) {
            this.appEntity.entityExtraData[currentExtraAttrIndex].attributeValueId = selectedAttrValue
        } else {
            this.appEntity.entityExtraData.push(newExtraData)
        }
    }

    notLookupExtraAttributeOnChange(value,extraAttrDefinition:FilteredExtraAttribute){

        if(!this.appEntity.entityExtraData)  this.appEntity.entityExtraData = []

        this.appEntity.entityExtraData = this.appEntity.entityExtraData.filter(elem=>elem.attributeId !== extraAttrDefinition.attributeId)
    ...
    resetExtraAttributeSelectedValue(extraAttr :FilteredExtraAttribute){
        ...
        this.appEntity.entityExtraData = this.appEntity.entityExtraData.filter(item=>item.attributeId !== extraAttr.attributeId)
    }
    ...
    openCreateAppEntityListModal(extraAttr: FilteredExtraAttribute ) {
        ...
        let subs :Subscription = this.bsModalService.onHidden.subscribe(()=>{
            this._extraAttributeDataService.getExtraAttributeLookupData(extraAttr.entityObjectTypeCode,extraAttr.lookupData)
