...
import {  SycEntityObjectTypesServiceProxy, TreeNodeOfGetSycEntityObjectTypeForViewDto } from '@shared/service-proxies/service-proxies';
...
export class SelectAppItemTypeComponent extends AppComponentBase implements OnInit {
    savedId : number
    ...
    getAppItemTypesList(searchQuery?:string){
        ...
        .subscribe((result)=>{
            ...
            this.showMoreListDataButton = this.allRecords.length < this.totalCount
            this.active = true;
            this.loading=false;
        })
    ...
    loadAppItemTypesNode($event :{node : TreeNodeOfGetSycEntityObjectTypeForViewDto }){
        if ($event.node) {
            ...
            .subscribe((res)=>{
                ...
                if(this.savedId){
                    res.forEach(item=>{
                        if(item.data.sycEntityObjectType.id == this.savedId) this.checkItemSelection(item)
                    })
                }
    ...
    cachRecordWithNodes(node:TreeNodeOfGetSycEntityObjectTypeForViewDto){
        const alreadyExit : boolean = !!this.loadedChildrenRecords.filter((elem)=>{
            return elem.data.sycEntityObjectType.id == node.data.sycEntityObjectType.id
        }).length
        if( !alreadyExit ){
    ...
    preventSelectNonLeaf(node : TreeNodeOfGetSycEntityObjectTypeForViewDto ){
        if(this.selectedRecord?.data?.sycEntityObjectType?.id == node?.data?.sycEntityObjectType?.id){
    ...
    filterAppitemTypes(searchQuery:string,list:TreeNodeOfGetSycEntityObjectTypeForViewDto[]){
        ...
        list.forEach((node)=>{
            ...
            const item : TreeNodeOfGetSycEntityObjectTypeForViewDto = new TreeNodeOfGetSycEntityObjectTypeForViewDto()
            item.init({...node,children:undefined})
            if(item?.children) item.children = this.filterAppitemTypes( searchQuery, node.children )
