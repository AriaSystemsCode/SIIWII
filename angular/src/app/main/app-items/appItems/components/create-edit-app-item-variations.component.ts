...
let index = this.activeAttachmentOption.attachmentSrcs?.length ? this.activeAttachmentOption.attachmentSrcs?.length-1 : 0;
    ...
    combine(index: number, _variation: VariationItemDto, oldAttributes: VariationItemDto[]) {
        ...
        const createNewVariation = (attrLabel: string, attrCode: string, attrId?: number) => {
            ...
            const entityExtraData = new AppEntityExtraDataDto({
                ...
                id: currentExtraAttr.id,
                entityId: currentExtraAttr.entityId,
                entityObjectTypeCode:currentExtraAttr.name.toLocaleLowerCase().includes('color')?'COLOR':'SIZE',
                attributeValueFkName: currentExtraAttr.attributeValueFkName,
                entityObjectTypeName: currentExtraAttr.entityObjectTypeName,
                entityObjectTypeId: currentExtraAttr.entityObjectTypeId,
            ...
            if (index < totalSelectedExtraAttributes - 1) {
                ...
                // ✅ Sort entityExtraData based on original BE order (if available)
                if (oldAttributes?.length > 0) {
                    const backendOrder = oldAttributes[0].entityExtraData.map(item => item.attributeId);
                    variation.entityExtraData.sort((a, b) =>
                        backendOrder.indexOf(a.attributeId) - backendOrder.indexOf(b.attributeId)
                    );
                }
                ...
                let matchedExisting: VariationItemDto;
                if (!this.appItem?.id) {
        ...
        // Ensure selectedValues are populated from displayed values if missing
        ...
        // Iterate over all selected values for current attribute
        currentExtraAttr.selectedValues.forEach(attrId => {
            let attrOptionData =
                currentExtraAttr.lookupData?.find(item => item.value == attrId) ||
