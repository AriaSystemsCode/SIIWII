...
import {
    ...
    CreateOrEditAppItemSelectorDto,
    GetAllAppItemsInput,
...
} from "../../app-items-browse/models/app-item-browse-inputs.model";
...
export class MyAppItemsComponent
    extends AppComponentBase
    implements AfterViewInit
{
    @ViewChild("AppItemsBrowseComponent")
    appItemsBrowseComponent: AppItemsComponent;
    @ViewChild("LinesheetModal", { static: true })
    linesheetModal: ModalDirective;
    ...
    ngAfterViewInit(): void {
        ...
        const pageMainFilters: SelectItem[] = [
            ...
            // { label:'Shared Restricted', value: ItemsFilterTypesEnum.MyListing },
            // { label:'Shared Public', value: ItemsFilterTypesEnum.MyListing },
            // { label:'Private', value: ItemsFilterTypesEnum.MyListing },
            // { label:'Hidden', value: ItemsFilterTypesEnum.MyListing },
        ];
        const filtersFlags: AppItemsBrowseComponentFiltersDisplayFlags =
            new AppItemsBrowseComponentFiltersDisplayFlags();
        const statusesFlags: AppItemsBrowseComponentStatusesFlags =
            new AppItemsBrowseComponentStatusesFlags();
        const actionsMenuFlags: AppItemsBrowseComponentActionsMenuFlags =
    ...
    selectAllHandler() {
        const body: GetAllAppItemsInput = new GetAllAppItemsInput({
            ...this.appItemsBrowseComponent.filterBody,
            maxResultCount:
                this.appItemsBrowseComponent.primengTableHelper
                    .totalRecordsCount,
        });
        this.appItemSelectorsServiceProxy
            .selectAll(this.multiSelectionInfo.sessionSelectionKey, body)
    ...
    invertHandler() {
        ...
        this.appItemSelectorsServiceProxy
            .invert(this.multiSelectionInfo.sessionSelectionKey, body)
            .subscribe((totalSelection) => {
                this.multiSelectionInfo.totalCount = totalSelection;
                this.reloadPage();
            });
    ...
    selectItem(item: GetAppItemForViewDto) {
        const body = new CreateOrEditAppItemSelectorDto({
            key: this.multiSelectionInfo.sessionSelectionKey,
            selectedId: item.appItem.id,
            id: 0,
        });
        this.appItemSelectorsServiceProxy.createOrEdit(body).subscribe(
            (totalSelection) => {
                this.multiSelectionInfo.totalCount = totalSelection;
                item.selected = true;
            },
    ...
    unSelectItem(item: GetAppItemForViewDto) {
        this.appItemSelectorsServiceProxy
            .delete(
                this.multiSelectionInfo.sessionSelectionKey,
                item.appItem.id,
                undefined
            )
            .subscribe(
    ...
    resetSelection() {
        return this.appItemSelectorsServiceProxy
            .deleteAllTempWithKey(
                this.multiSelectionInfo.sessionSelectionKey,
                undefined,
                undefined
            )
    ...
    showLinesheetWizardModal() {
        this.getAllAppItemsInputForLineSheet = new GetAllAppItemsInput({
            ...this.appItemsBrowseComponent.filterBody,
            maxResultCount: this.multiSelectionInfo.totalCount,
        });
