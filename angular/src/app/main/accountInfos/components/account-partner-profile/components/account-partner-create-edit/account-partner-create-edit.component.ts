...
@Component({
    selector: 'app-account-partner-create-edit',
    templateUrl: './account-partner-create-edit.component.html',
    styleUrls: ['./account-partner-create-edit.component.scss'],
})
export class AccountPartnerCreateEditComponent
    extends AppComponentBase
    implements OnInit
{
    @ViewChild('accountPartnerForm', { static: true })
    accountPartnerForm: NgForm;

    @Input() accountId?: number;
    @Input() partnerType: AccountPartnerType = AccountPartnerType.Manual;
    @Input() mode: AccountPartnerMode = AccountPartnerMode.Create;
    ...
    accountInfoTemp: CreateOrEditAccountInfoDto =
...
otherImageSlots = [
  ...
  if (this.isEdit && this.accountId) {
    ...
    get isCreate(): boolean {
        return this.mode === AccountPartnerMode.Create;
    }

    get isEdit(): boolean {
        return this.mode === AccountPartnerMode.Edit;
    }
    ...
    private initDto(): void {
        ...
        this.accountInfoTemp.contactAddresses = [];
        this.accountInfoTemp.contactPaymentMethods = [];
        this.accountInfoTemp.branches = [];
    }
    ...
    private prepareCreateMode(): void {
        this.accountInfoTemp.accountLevel = AccountLevelEnum.Manual;
    ...
    private getContactDefaults(): void {
        this._accountsServiceProxy.getContactDefaults().subscribe((res) => {
            ...
            if (!this.accountInfoTemp.paymentTermsId) {
                this.accountInfoTemp.paymentTermsId = this.paymentTermsId;
            }

            if (!this.accountInfoTemp.shipViaId) {
                this.accountInfoTemp.shipViaId = this.shipViaId;
            }
        });
    }
    ...
    getAccountDataForEdit(): void {
        ...
        this._accountsServiceProxy
            ...
            .subscribe((res) => {
                ...
                this.accountInfoTemp = CreateOrEditAccountInfoDto.fromJS(
                    res.accountInfo
                );
                ...
                this.accountInfoTemp.paymentTermsId =
                    res.accountInfo?.paymentTermsId || this.paymentTermsId;

                this.accountInfoTemp.shipViaId =
                    res.accountInfo?.shipViaId || this.shipViaId;
    ...
    getAccountTypes(): void {
        this._appEntitiesServiceProxy
            ...
            .subscribe((result) => {
                ...
                if (this.isCreate && !this.accountInfoTemp.accountTypeId) {
                    this.accountInfoTemp.accountTypeId = 19;
                    this.accountInfoTemp.accountType = 'Business';
                }
            });
    ...
    private setDefaultPhoneTypes(): void {
        ...
        if (!this.accountInfoTemp.phone1TypeId) {
            this.accountInfoTemp.phone1TypeId = this.allPhoneTypes[0].value;
        }
    }
    ...
    private async setManualAccCode(): Promise<void> {
        ...
        if (this.accountInfoTemp.code) return;

        this.hasRequestedManualCode = true;

        const accountTypeCode =
            this.accountInfoTemp.accountTypeId === 21
                ? 'PERSONAL'
                : this.accountInfoTemp.accountTypeId === 20
                ? 'GROUP'
                : 'BUSINESS';
        ...
        if (sequence && !this.accountInfoTemp.code) {
            this.accountInfoTemp.code = 'M' + sequence;
    ...
    resetFormData(): void {
        ...
        if (this.isEdit && this.getForEditResult) {
            ...
            setTimeout(() => {
                this.accountPartnerForm?.form?.patchValue(
        ...
        this.initDto();
        this.prepareCreateMode();
        this.accountPartnerForm?.resetForm(this.accountInfoTemp);
    }
    ...
    save(): void {
  // if (!this.accountPartnerForm?.form?.valid) return;
  ...
  this.accountInfoTemp.accountLevel = this.isManual
  ...
  this._accountsServiceProxy
    .createOrEditAccount(this.accountInfoTemp)
    ...
    private applyConnectedLimitedEdit(): void {
        ...
        const current = this.accountInfoTemp as any;
  ...
  this.changeTouchState();
    ...
    removeOtherImage(index: number): void {
        ...
        this.accountInfoTemp.entityAttachments =
            this.accountInfoTemp.entityAttachments?.filter(
                (x: any) => x.index !== index
            ) ?? [];
    ...
    private upsertAttachmentFromUpload(
        ...
        this.accountInfoTemp.entityAttachments.push(attachment);
    }
  ...
  const attachments: any[] = this.accountInfoTemp.entityAttachments ?? [];
...
imageBrowseDone(
  ...
  this.uploader.onSuccessItem = (_item, response) => {
    ...
    const current = this.accountInfoTemp.entityAttachments
...
removeImage(type: 'LOGO' | 'BANNER' | 'IMAGE', index: number): void {
  ...
  this.accountInfoTemp.entityAttachments =
    this.accountInfoTemp.entityAttachments?.filter((x: any) => {
...
private upsertAttachment(attachment: AppEntityAttachmentDto): void {
  ...
  this.accountInfoTemp.entityAttachments =
    this.accountInfoTemp.entityAttachments?.filter((x: any) => x.index !== index) ?? [];
