...
import {  CreateOrEditSycEntityObjectCategoryDto, SycEntityObjectCategoriesServiceProxy, SycEntityObjectCategoryDto, TreeNodeOfGetSycEntityObjectCategoryForViewDto } from '@shared/service-proxies/service-proxies';
...
export class SelectCategoriesDynamicModalComponent extends AppComponentBase implements OnInit {
    ...
    openCreateOrEditModal({ category,parentCategory, addAsChild = false }: { parentCategory?: SycEntityObjectCategoryDto; category?: SycEntityObjectCategoryDto; addAsChild?: boolean; } = {}) : void {
        ...
        if(addAsChild) { // add new child
            category =  new CreateOrEditSycEntityObjectCategoryDto()
            if(parentCategory) category.parentId = parentCategory.id
        } else {
            if (!category) category =  new CreateOrEditSycEntityObjectCategoryDto()
            if (parentCategory) category.parentId = parentCategory.id
        }
        if(parentCategory) initialModalData.parentCategory = JSON.parse(JSON.stringify(parentCategory))
        ...
        this.createOrEditModalRef = this._BsModalService.show(CreateOrEditCategoryDynamicModalComponent,config)
    ...
    isSelected = (id:number) : boolean => !!this.lastSelectedRecords.filter(item=>item.data.sycEntityObjectCategory.id == id)[0]
    ...
    addAsNewChild(node:TreeNodeOfGetSycEntityObjectCategoryForViewDto){
        this.openCreateOrEditModal({ parentCategory: node.data.sycEntityObjectCategory, addAsChild: true })
    }

    editCategory(node){

        let category = node.data.sycEntityObjectCategory
        let parentCategory = node.parent? node.parent.data.sycEntityObjectCategory : undefined
        this.openCreateOrEditModal({category,parentCategory})
    }
    deleteSycEntityObject(node: TreeNodeOfGetSycEntityObjectCategoryForViewDto): void {

        var isConfirmed: Observable<boolean>;
        let message = node.leaf ? "AreYouSure" : "AreYouSure,AllSubCategoriesWillBeDeleted";
       isConfirmed   = this.askToConfirm(message,"");

   isConfirmed.subscribe((res)=>{
      if(res){
                    this._sycEntityObjectCategoriesServiceProxy.delete(node.data.sycEntityObjectCategory.id)
                    .subscribe(() => {
                        if((node as any).parent) {
                            (node as any).parent.children = (node as any).parent.children.filter(item=>item.data.sycEntityObjectCategory.id != node.data.sycEntityObjectCategory.id)
                        } else {
                            this.displayedRecords = this.displayedRecords.filter(item=>item.data.sycEntityObjectCategory.id != node.data.sycEntityObjectCategory.id)
                        }
    ...
    getCategoriesList(){
        ...
        .subscribe((result:{items:TreeNodeOfGetSycEntityObjectCategoryForViewDto[],totalCount:number})=>{
            ...
            if(this.savedIds?.length){
                currentLoadedItemsAfterExludingSelections = result.items.filter((item)=>{
                    return !this.savedIds.includes(item.data.sycEntityObjectCategory.id)
                })
            ...
            currentLoadedItemsAfterExludingSelections.map((record)=>{
                ...
                const cachedItem : TreeNodeOfGetSycEntityObjectCategoryForViewDto = this.loadedChildrenRecords.filter((selectedRecord:TreeNodeOfGetSycEntityObjectCategoryForViewDto)=>{
                    const isCached : boolean = selectedRecord.data.sycEntityObjectCategory.id == record.data.sycEntityObjectCategory.id
                    return isCached
                })[0]
    ...
    loadCategoriesNode($event: { node : TreeNodeOfGetSycEntityObjectCategoryForViewDto }){
        if ($event.node) {
    ...
    cachRecordWithNodes(node:TreeNodeOfGetSycEntityObjectCategoryForViewDto){
        const alreadyExit : boolean = !!this.loadedChildrenRecords.filter((elem)=>{
            return elem.data.sycEntityObjectCategory.id == node.data.sycEntityObjectCategory.id
        }).length
        if( !alreadyExit ){
    ...
    filterCategories(searchQuery:string,list:TreeNodeOfGetSycEntityObjectCategoryForViewDto[]){
        const filterList :TreeNodeOfGetSycEntityObjectCategoryForViewDto[]= []
        list.forEach((node)=>{
            if(!node?.label?.toLowerCase()?.includes(searchQuery?.toLowerCase())) return
            const item : TreeNodeOfGetSycEntityObjectCategoryForViewDto = new TreeNodeOfGetSycEntityObjectCategoryForViewDto()
            item.init({...node,children:undefined})
            if(item?.children) item.children = this.filterCategories( searchQuery, node.children )
