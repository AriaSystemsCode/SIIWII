...
namespace onetouch.Accounts
{
    ...
    .OrderByDescending(z => z.EntityLastModificationTime);
        ...
        //X527[End]
        public async Task<PagedResultDto<GetAccountForViewDto>> 
            GetAll(GetAllAccountsInput input)
        {
            var stopwatch = new System.Diagnostics.Stopwatch();
            stopwatch.Start();
                ...
                try
                {
                    ...
                    var filteredAccounts = _appContactRepository.GetAll().AsNoTracking()
                            ...
                            .WhereIf(input.FilterType <= 1 && input.FilterType != 6,
                             ...
                             (x.EntityFk.TenantOwner == AbpSession.TenantId ?
                                x.EntityFk.EntityObjectTypeId != personId : true) &&
                    ...
                    if (accountConnection != null && accountConnection.Id > 0)
                    {
                        ...
                        if (relationshipsList != null && relationshipsList.Count > 0)
                        {
                            ...
                            foreach (var relationshipInf in relationshipsList)
                            {
                                ...
                                connInfo.RequestorRole = relationshipInf.RequesterMarketplaceRole;
                                connInfo.RecipientRole = relationshipInf.RecipientMarketplaceRole;
...
.FirstOrDefaultAsync(r => r.TenantId == tenant.Id && r.Name == StaticRoleNames.Tenants.Admin);
                    ...
                    if (originalContact != null)
                    {
                        ...
                        if (existed == null)//&& sync == false)
                        {
                                ...
                                && tenantId != AbpSession.TenantId)
                            {
                                ...
                                createOrEditAccountInfoDto.Code = existed.Code;//tenantObj.TenancyName.Trim() +
                                                                               //}
                                createOrEditAccountInfoDto.AccountLevel = AccountLevelEnum.Manual;
                                ...
                                if (createOrEditAccountInfoDto.EntityExtraData != null)
                                {
                                    foreach (var parentExtrData in createOrEditAccountInfoDto.EntityExtraData)
                                    {
                                        ...
                                        parentExtrData.EntityId = existed.EntityId;
                                        //if (originalContact.EntityObjectTypeId == presonEntityObjectTypeId &&
                                        //      parentExtrData.AttributeId == 715)
                                        {
                                            var existingAttr = existed.EntityFk.EntityExtraData.Where(z => z.AttributeId == parentExtrData.AttributeId).FirstOrDefault();
                                            if (existingAttr != null)
                                            {
                                                parentExtrData.Id = existingAttr.Id;
                                                parentExtrData.AttributeValue = existingAttr.AttributeValue;
                                            }
                                            else
                                                parentExtrData.Id = 0;
                                        }
                   ...
                   .FirstOrDefaultAsync(x => x.TenantId == tenantId && x.SSIN == originalContact.SSIN);
                    ...
                    else
                    {
                          ...
                          //  originalContact.TenantOwner != AbpSession.TenantId)
                        {
                            ...
                            //        property.SetValue(createOrEditAccountInfoDtoObj, propertyValue, null);
                            //    }
                            //}
                            createOrEditAccountInfoDtoObj.UseDTOTenant = true;
                            createOrEditAccountInfoDtoObj.TenantId = tenantId;
                            createOrEditAccountInfoDtoObj.Id = existed.Id;
                            createOrEditAccountInfoDtoObj.Code = existed.Code;
                            createOrEditAccountInfoDtoObj.ParentId = existed.ParentId;
                            createOrEditAccountInfoDtoObj.AccountId = existed.AccountId;
                            createOrEditAccountInfoDtoObj.AccountLevel = AccountLevelEnum.Manual;
                            ...
                            //if (originalContact.EntityObjectTypeId == presonEntityObjectTypeId)
                            createOrEditAccountInfoDtoObj.ContactRecordType = "C";
                            savedAccountSrc = await CreateOrEditAccount(createOrEditAccountInfoDtoObj);
                            var accountSaved = savedAccountSrc;
                            if (accountSaved != null && accountSaved.AccountInfo.Id > 0)
                            {
                                if (originalContact.ContactAddresses != null && originalContact.ContactAddresses.Count > 0)
                    ...
                    if (originalContact != null && originalContact.TenantOwner != null )//&& sync == false)
                    {
                        ...
                        { 
                            ...
                            var adminRole = await _roleManager.Roles
                                    .FirstOrDefaultAsync(r => r.TenantId == int.Parse(originalContact.TenantOwner.ToString()) && r.Name == StaticRoleNames.Tenants.Admin);
                            ...
                            if (adminRole != null)
                            {
                                ...
                                adminUser = await _userManager.Users
                                    .Where(u => u.TenantId == int.Parse(originalContact.TenantOwner.ToString()))
                                    .Where(u => u.Roles.Any(r => r.RoleId == adminRoleId))
                                    .FirstOrDefaultAsync();
                            }
                            if (adminUser != null)
                        ...
                        if (PublishContactFortDisconnectFromTenant != null)
                        {
                            ...
                            // if (profileContactofOtherTenant != null && profileContactofOtherTenant.TenantId != null)
                            {
                                ...
                                if (tenantObject != null)
                                {
                                    ...
                                    var adminRole = await _roleManager.Roles
                                            .FirstOrDefaultAsync(r => r.TenantId == int.Parse(PublishContactFortDisconnectFromTenant.TenantOwner.ToString()) && r.Name == StaticRoleNames.Tenants.Admin);
                                    ...
                                    if (adminRole != null)
                                    {
                                        ...
                                        adminUser = await _userManager.Users
                                            .Where(u => u.TenantId == int.Parse(PublishContactFortDisconnectFromTenant.TenantOwner.ToString()))
                                            .Where(u => u.Roles.Any(r => r.RoleId == adminRoleId))
                                            .FirstOrDefaultAsync();
                                    }
                                    if (adminUser != null)
            ...
            if (currentRelation != null)
            {
                ...
                if (accountConnection != null && accountConnection.Id > 0)
                {
                    ...
                    var relationshipsList = await _appContactRelationshipInfoRepository.GetAll()
                            ...
                            || (z.RecipientContactSSIN == ssin && z.RequesterContactSSIN == originalPublishContactFortCurrTenant.SSIN)) && z.EntityObjectStatusId != null && z.EntityObjectStatusId != inActiveRelationshipStatusId)
                           ).OrderByDescending(z => z.CreationTime).ToListAsync();
                    if (relationshipsList != null && relationshipsList.Count > 0)
                    {
            ...
            using (UnitOfWorkManager.Current.DisableFilter(AbpDataFilters.MustHaveTenant, AbpDataFilters.MayHaveTenant))
            {
              ...
              .Where(x => x.Id == input).FirstOrDefaultAsync();
                if (contact != null)
                {
                    ...
                    if (returnObject.EntityExtraData != null && returnObject.EntityExtraData.Count>0)
                    {
                        var userAttribute = returnObject.EntityExtraData
                            .Where(z => z.AttributeId == 715).FirstOrDefault();
                        if (userAttribute != null && userAttribute.AttributeValue == "0")
                            userAttribute.AttributeValue = null;
                    }
     ...
     languageIds, DeptIds, countries);
        ...
        public async Task<ExcelLogDto> _SaveFromExcel(AccountExcelResultsDTO accountExcelResultsDTO)
        //MMT2
        {
            #region get lists
